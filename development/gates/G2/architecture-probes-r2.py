"""Independent G2 revision-2 probes; original negative cases retained."""
import hashlib
import io
...
from agent_architect import core, cli
...
def make_council(root, variant):
    ...
    for role in ("one", "two", "three"):
        body = "Verdict: APPROVE\nCandidate SHA-256: " + candidate_a_hash + "\nSynthetic test report.\n"
...
with tempfile.TemporaryDirectory(prefix="aa-g2-architecture-") as name:
    ...
    observed = dict(error=error, head_changed=p.head()[0] != before,
                    ...
                    status=(result or {}).get("status"), commit_matches=(result or {}).get("commit") == p.head()[0])
    record("post_head_fsync_failure", "explicit committed-durability-uncertain with visible revision/hash", observed,
           error is None and observed["status"] == "committed-durability-uncertain"
           and observed["commit_matches"] and observed["head_changed"] and observed["revision"] == 2)


    p = project(base / "cli_post_head")
    incoming = base / "cli_record.json"
    incoming.write_text(json.dumps(note("CliPublished")))
    replaced[0] = False
    stdout, stderr = io.StringIO(), io.StringIO()
    argv = ["--project", str(p.path), "put", str(incoming), "--expect-revision", "1"]
    with patch.object(core.os, "replace", side_effect=replacing), patch.object(core.os, "fsync", side_effect=syncing):
        with redirect_stdout(stdout), redirect_stderr(stderr):
            exit_code = cli.main(argv)
    response = json.loads(stdout.getvalue())
    observed = dict(exit=exit_code, status=response.get("status"), revision=response.get("revision"),
                    commit_matches=response.get("commit") == p.head()[0], stderr=stderr.getvalue())
    record("cli_post_head_fsync_failure", "exit 0 with explicit uncertainty and visible revision/hash", observed,
           observed == dict(exit=0, status="committed-durability-uncertain", revision=2, commit_matches=True, stderr=""))
    stdout, stderr = io.StringIO(), io.StringIO()
    with redirect_stdout(stdout), redirect_stderr(stderr):
        exit_code = cli.main(argv)
    response = json.loads(stderr.getvalue())
    observed = dict(exit=exit_code, stale="Stale revision" in response.get("message", ""),
                    revision=p.read()["revision"], commits=p.audit()["commits"])
    record("retry_after_visible_commit", "reject stale retry without duplicate commit", observed,
           observed == dict(exit=2, stale=True, revision=2, commits=2))
