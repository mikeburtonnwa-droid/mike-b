"""Independent G2 revision-2 product-review evidence; all project mutations use temp dirs."""
import hashlib
import json
import os
...
events, checks = [], []
original_names = ['product.md', 'product-checks.py', 'product-cli-evidence.json']
...
with tempfile.TemporaryDirectory(prefix='aa-g2-product-') as temporary:
    ...
    def build_council(first_body='Verdict: APPROVE\n', report_hash=None, dispatch_candidate='candidate.json'):
        bodies = [first_body, 'Verdict: APPROVE\n', 'Verdict: APPROVE\n']
        report_hash = report_hash or sha(gate / 'candidate.json')
        bodies = [body.replace('\n', f'\nCandidate SHA-256: {report_hash}\n', 1) for body in bodies]
        reviews = []
        ...
        for role, body in zip(roles, bodies):
            event('orchestrator', role, 'dispatch', f'Review synthetic {dispatch_candidate}.')
    ...
    build_council(report_hash='0' * 64)
    result = council()
    check('synthetic_council_wrong_candidate_rejected', result['exit_code'] == 2, parsed(result))
    build_council(dispatch_candidate='candidate-old.json')
    result = council()
    check('synthetic_council_wrong_dispatch_rejected', result['exit_code'] == 2, parsed(result))
    build_council('Verdict: APPROVE\nVerdict: DENY\n')
    result = council()
    check('synthetic_council_ambiguous_header_rejected', result['exit_code'] == 2, parsed(result))
    build_council('Verdict: APPROVE\n\n## Findings\nHistorical example: Verdict: DENY\n')
    result = council()
    check('synthetic_council_body_example_is_not_verdict', result['exit_code'] == 0, parsed(result))

check('candidate_after', all((ROOT / p).is_file() and sha(ROOT / p) == h for p, h in manifest.items()),
      {'entries': len(manifest)})
check('original_review_evidence_preserved', all(sha(OUT / name) == value for name, value in original_hashes.items()), original_hashes)
payload = dict(checks=checks, commands=events)
(OUT / 'product-cli-evidence-r2.json').write_text(json.dumps(payload, indent=2) + '\n')
