"""Independent G3 revision-3 probes; original scenarios and replay negatives retained."""
import copy
import hashlib
import io
import json
...
def capture_case(p, rid, run_at, passed=True):
    run_on = datetime.fromisoformat(run_at).astimezone(timezone.utc).date().isoformat()
    payload = dict(environment="test", run_on=run_on, run_at=run_at, run_id=str(uuid.uuid4()),
                   command="Independent synthetic ordering scenario", exit_code=0, criterion=CRITERION,
                   target_hashes=w.fingerprints(p.read()["records"], ["REQ","COMP"]),
                   brief_hash=w.brief_hash(p.read()), expected={"decisions":1},
                   observed={"decisions":1 if passed else 2})
    capture(p, "SRC_"+rid, json.dumps(payload).encode(), kind="query-result", date=run_on)
    return w.evaluate(p, dict(id=rid, title=rid, description="Independent synthetic execution ordering",
                      targets=["REQ","COMP"], result_source="SRC_"+rid,
                      valid_until="2026-10-09", criterion=CRITERION), revision(p))

with tempfile.TemporaryDirectory(prefix="aa-g3-architecture-") as dirname:
    root = Path(dirname)
    counter = [0]
    def fresh(evaluation=True):
        counter[0] += 1
        return ready_project(root / ("project-" + str(counter[0])), evaluation=evaluation)
    ...
    result = dict(environment="test", run_on=DATE, command="Synthetic recovery observation", exit_code=0,
                  criterion="Service recovered", target_hashes=w.fingerprints(p.read()["records"], ["COMP"]), brief_hash=w.brief_hash(p.read()),
                  run_id=str(uuid.uuid4()), run_at=DATE+"T00:01:00+00:00",
    ...
    p = fresh()
    failed = run_result(p, "SAME_DAY_FAIL", observed={"decisions":2}, run_on=DATE)
    before_replay = w.readiness(p.read(), DATE, "test", "HANDOFF")
    observed("same_day_failure_before_replay", "later same-day failure blocks readiness",
             dict(status=before_replay["status"], outcome=failed["outcome"]),
             before_replay["status"] == "fail" and failed["outcome"] == "fail")
    blocked = False
    try:
        replay = w.evaluate(p, dict(id="REPLAY_OLD_PASS", title="Repeat import", description="Same old captured run",
                            targets=["REQ","COMP"], result_source="RESULT_EV1", valid_until="2026-10-09",
                            criterion=CRITERION), revision(p))
    except core.Invalid:
        blocked = True
    after_replay = w.readiness(p.read(), DATE, "test", "HANDOFF")
    value = dict(replay_rejected=blocked, readiness=after_replay["status"], selected=after_replay["evaluations"],
                 original_source="RESULT_EV1", latest_failed_run="SAME_DAY_FAIL")
    observed("old_pass_reimport_after_same_day_failure",
             "re-importing the original result must not resolve a later failed run", value,
             after_replay["status"] == "fail")

    p = fresh()
    release(p)
    p.put(incident(p) | dict(diagnosis="Synthetic defect", action="Restore service", follow_up="Check next run"), revision(p))
    run_result(p, "OLD_RECOVERY", criterion="Service recovered", targets=["COMP"])
    run_result(p, "FAILED_RECOVERY", criterion="Service recovered", targets=["COMP"], observed=False)
    probe_reject("old_recovery_after_same_day_failure", p,
                 lambda: w.close_incident(p, "INC1", "OLD_RECOVERY", DATE, revision(p)))
    blocked = False
    try:
        w.evaluate(p, dict(id="REPLAY_RECOVERY", title="Repeat import", description="Same old recovery run",
                          targets=["COMP"], result_source="RESULT_OLD_RECOVERY", valid_until="2026-10-09",
                          criterion="Service recovered"), revision(p))
    except core.Invalid:
        blocked = True
    if blocked:
        observed("old_recovery_reimport", "old result replay cannot close incident", "replay rejected", True)
    else:
        probe_reject("old_recovery_reimport", p,
                     lambda: w.close_incident(p, "INC1", "REPLAY_RECOVERY", DATE, revision(p)))


    p = fresh()
    run_result(p, "ALIAS_LATEST_FAILURE", observed=False)
    src = p.read()["records"]["RESULT_EV1"]
    original_bytes = (p.path / src["blob"]).read_bytes()
    for alias, content in [("CONTENT_ALIAS", original_bytes),
                           ("FORMATTED_ALIAS", json.dumps(json.loads(original_bytes), indent=4).encode())]:
        capture(p, alias, content, kind="query-result")
        probe_reject("replay_" + alias.lower(), p,
                     lambda alias=alias: w.evaluate(p, dict(id=alias+"_EV", title="Replay alias",
                         description="Original run identity and time unchanged", targets=["REQ","COMP"],
                         result_source=alias, valid_until="2026-10-09", criterion=CRITERION), revision(p)))
    state = w.readiness(p.read(), DATE, "test", "HANDOFF")
    observed("alias_replay_keeps_failure", "readiness remains failed after original-run aliases",
             state["status"], state["status"] == "fail")

    p = fresh(evaluation=False)
    capture_case(p, "EXECUTED_LATER_FAILURE", DATE+"T12:00:00+00:00", passed=False)
    capture_case(p, "IMPORTED_LATER_OLD_PASS", DATE+"T11:00:00+00:00")
    state = w.readiness(p.read(), DATE, "test", "HANDOFF")
    observed("older_run_backfill", "late import of older execution cannot resolve later failure",
             dict(status=state["status"], selected=state["evaluations"]), state["status"] == "fail")
    capture_case(p, "ACTUALLY_LATER_PASS", DATE+"T13:00:00+00:00")
    state = w.readiness(p.read(), DATE, "test", "HANDOFF")
    observed("strictly_later_execution", "newer execution resolves failure",
             dict(status=state["status"], selected=state["evaluations"]),
             state["status"] == "pass" and state["evaluations"] == ["ACTUALLY_LATER_PASS"])

    p = fresh(evaluation=False)
    capture_case(p, "TIED_FAILURE", DATE+"T12:00:00+00:00", passed=False)
    capture_case(p, "TIED_PASS_OTHER_OFFSET", DATE+"T07:00:00-05:00")
    state = w.readiness(p.read(), DATE, "test", "HANDOFF")
    observed("equivalent_timezone_tie", "tied failure remains blocking across equivalent timezones",
             state["status"], state["status"] == "fail")
    capture_case(p, "AFTER_TIE", DATE+"T12:00:01+00:00")
    state = w.readiness(p.read(), DATE, "test", "HANDOFF")
    observed("strictly_after_tie", "strictly later success resolves tied failure",
             dict(status=state["status"], selected=state["evaluations"]),
             state["status"] == "pass" and state["evaluations"] == ["AFTER_TIE"])
