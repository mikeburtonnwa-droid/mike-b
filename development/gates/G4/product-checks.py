...
import hashlib,html,json,os,re,shutil,sqlite3,subprocess,sys,tempfile,zipfile
...
with tempfile.TemporaryDirectory(prefix='aa-g4-product-') as tmp:
    ...
    check('first_render_rework_is_readable',r['exit_code']==0 and 'No local simulation is activated.' in first_render and 'Current task: ' in first_render and 'Next action: **'+f['checkpoint']['next_action']+'**' in html.unescape(first_render) and 'Covered means an attributed account' in first_render and '{&quot;current_task&quot;' not in first_render,dict(characters=len(first_render),activation_line=first_render.splitlines()[4]))
    ...
    check('saved_query_routes_match_sponsor',{(v['tenant'],v['request_id']):v['routing_disposition'] for v in rows['Q04']}==expected and all(v['tenant']==v['customer_tenant'] for v in rows['Q04']),dict(rows=rows['Q04']))
