...
def record(rid, kind, **fields):
    return dict(id=rid, type=kind, title=rid, description='Synthetic order-triage exercise', status='active') | fields
...
def build(r):
    ...
    for rid, kind, actor, system, boundary, variant, edges in specs:
        values.append(record(rid, 'process', view='current', kind=kind, actor=actor, system=system,
                             ...
                             variant=variant, evidence=evidence('S01', 'S03', 'S04'),
    ...
    r.run('save handoff checkpoint', lambda: p.checkpoint(dict(current_task='Bounded simulation complete',
        ...
        relevant_ids=['HANDOFF', 'INC_FEED', 'Q_ROUTES', 'COMP_ROUTER'], stage='operating-handoff'), r.revision(), ACTOR))
